                      ...
                      rostring mlFname, rostring srcFname)
{
  ...
  format_to(dcl,
    "val {0}ParseTables: Parsetables.tParseTables\n"
    "val {0}UserActions: Useract.tUserActions\n",
    g.actionClassName
  );
  ...
  format_to(out,
    "(* {} *)\n"
    "(* *** DO NOT EDIT BY HAND *** *)\n"
    "(* automatically generated by gramanl, from {} *)\n"
    "\n"
  ...
  format_to(out, "let {}UserActions = {{\n", g.actionClassName);
  #define COPY(name) \
    out.append("  " #name " = " #name "Func;\n");
...
void emitMLDescriptions(GrammarAnalysis const &g, EmitCode &out)
{
  ...
  {
    out.append("let termNamesArray: string array = [|\n");
    ...
    out.append(
      "  \"\"   (* dummy final value for ';' separation *)\n"
      "|]\n"
      "\n"
    );
  }
  ...
  out.append(
    "let terminalDescriptionFunc (termId:int) (sval:tSemanticValue) : string =\n"
    "begin\n"
  ...
  {
    out.append("let nontermNamesArray: string array = [|\n");
  ...
  out.append(
    "let nonterminalDescriptionFunc (nontermId:int) (sval:tSemanticValue)\n"
    "  : string =\n"
    "begin\n"
  ...
  out.append(
    "let terminalNameFunc (termId:int) : string =\n"
    "begin\n"
    "  termNamesArray.(termId)\n"
    "end\n"
    "\n"
...
void emitMLActions(Grammar const &g, EmitCode &out, EmitCode &dcl)
{
  ...
  {FOREACH_OBJLIST(Production, g.productions, iter) {
    ...
    FOREACH_OBJLIST(Production::RHSElt, prod.right, rhsIter) {
      ...
      format_to(out, "  let {} = (Obj.obj svals.({}) : {}) in\n",
        elt.tag, index, typeString(elt.sym->type, elt.tag));
    }

    // give a name to the yielded value so we can ensure it conforms to
    // the declared type
    format_to(out, "  let __result: {} =", prod.left->type);
  ...
  out.append(
    "(fun _ -> (failwith \"bad production index\"))   (* no ; *)"
    "\n"
    "|]\n"
    "\n"
  );
  ...
  out.append(
    "let reductionActionFunc (productionId:int) (svals: tSemanticValue array)\n"
    "  : tSemanticValue =\n"
    "begin\n"
...
void emitMLFuncDecl(Grammar const &g, EmitCode &out, EmitCode &dcl,
                    char const *rettype, char const *params)
{
  format_to(out, "(*inline*) let {}: {} =", params, rettype);
}
                      ...
                      char const *templateCode, char const *actUpon)
{
  ...
  switch (whichFunc) {
    ...
    case 2:    // unspecified merge: warn, but then use left (arbitrarily)
      format_to(out,
        "      (Printf.printf \"WARNING: no action to merge nonterm %s\\n\"\n"
        "                     nontermNamesArray.({}));\n"
        "      (flush stdout);\n"
        "      left\n",
        switchVar
                 ...
                 char const *tableName)
{
  ...
  for (int i=0; i<size; i++) {
    ...
    if (printHex) {
      format_to(out, "0x{:02X}", table[i]);    // ML: not used
    }
