  ...
  rostring ccFname, rostring srcFname)
{
  ...
  string _base_hFname = sm_basename(hFname);
  fmt::format_args args = fmt::make_format_args(
      fmt::arg("hFname", hFname),
      fmt::arg("ccFname", ccFname),
      fmt::arg("srcFname", srcFname),
      fmt::arg("latchName", latchName),
      fmt::arg("base_hFname", _base_hFname),
      fmt::arg("g_actionClassName", g.actionClassName)
  );
  ...
  vformat_to(out,
    "// {ccFname}\n"
    "// *** DO NOT EDIT BY HAND ***\n"
    "// automatically generated by gramanl, from {srcFname}\n"
    "\n"
  ...
  #endif
    "\n"
    "#include \"{base_hFname}\"     // {g_actionClassName}\n"
  ...
  NOSOURCELOC(
    out.append(
      "// parser-originated location information is disabled by\n"
...
void emitUserCode(EmitCode &out, LocString const &code, bool braces)
{
  out.push_back('\n');
  if (code.validLoc()) {
    out.append(lineDirective(code.loc));
  }
...
void emitDescriptions(GrammarAnalysis const &g, EmitCode &out)
{
  ...
  format_to(out,
    "string {g_actionClassName}::terminalDescription(int termId, SemanticValue sval)\n"
    "{{\n"
  ...
  format_to(out,
    "string {g_actionClassName}::nonterminalDescription(int nontermId, SemanticValue sval)\n"
    "{{\n"
  ...
  format_to(out,
    "char const *{g_actionClassName}::terminalName(int termId)\n"
    "{{\n"
    "  return termNames[termId];\n"
    "}}\n"
    "\n"
    "char const *{g_actionClassName}::nonterminalName(int nontermId)\n"
    "{{\n"
...
void emitActions(Grammar const &g, EmitCode &out, EmitCode &dcl)
{
  ...
  {FOREACH_OBJLIST(Production, g.productions, iter) {
    ...
    format_to(out,
      "// {prod}\n"
      "inline {prod_left_type} {g_actionClassName}::{actionFuncName_prod}("
      SOURCELOC("SourceLoc loc"),
      fmt::arg("prod", prod.toString()),
      fmt::arg("prod_left_type", prod.left->type),
      fmt::arg("g_actionClassName", g.actionClassName),
      fmt::arg("actionFuncName_prod", actionFuncName(prod))
    );
    format_to(dcl,
      "  {} {}("
      SOURCELOC("SourceLoc loc"),
    ...
    FOREACH_OBJLIST(Production::RHSElt, prod.right, rhsIter) {
      ...
      format_to(out,
        "{} {}",
        typeString(elt.sym->type, elt.tag),
        elt.tag
      );
      format_to(dcl,
        "{} {}",
  ...
  format_to(out,
    "/*static*/ SemanticValue {g_actionClassName}::doReductionAction(\n"
    "  {g_actionClassName} *ths,\n"
    "  int productionId, SemanticValue const *semanticValues"
    SOURCELOC(",\n  SourceLoc loc")
    ")\n"
  ...
  FOREACH_OBJLIST(Production, g.productions, iter) {
    ...
    FOREACH_OBJLIST(Production::RHSElt, prod.right, rhsIter) {
      ...
      format_to(out, "({})", typeString(elt.sym->type, elt.tag));
      ...
      format_to(out, "(semanticValues[{}])", index);
    }

    out.push_back(')');     // end of argument list
  ...
  format_to(out,
    "\n"
    "UserActions::ReductionActionFunc {g_actionClassName}::getReductionAction()\n"
    "{{\n"
    "  return (ReductionActionFunc)&{g_actionClassName}::doReductionAction;\n"
    "}}\n",
    fmt::arg("g_actionClassName", g.actionClassName)
  );
}
...
void emitDupDelMerge(GrammarAnalysis const &g, EmitCode &out, EmitCode &dcl)
{
  out.append("// ---------------- dup/del/merge/keep nonterminals ---------------\n"
    "\n");
  ...
  format_to(out,
    "UserActions::ReclassifyFunc {0}::getReclassifier()\n"
    "{{\n"
    "  return (ReclassifyFunc)&{0}::reclassifyToken;\n"
    "}}\n",
    g.actionClassName
  );
}
  ...
  char const* rettype, char const* params)
{
  fmt::format_args args = fmt::make_format_args(
    ...
    fmt::arg("rettype", rettype),
    fmt::arg("g_actionClassName", g.actionClassName)
  );
  vformat_to(out, "inline {rettype} {g_actionClassName}::{params}", args);
  vformat_to(dcl, "  inline {rettype} {params};\n", args);
}
                    ...
                    char const *templateCode, char const *actUpon)
{
  format_to(out,
    ...
    "{{\n"
    "  switch ({}) {{\n",
    replace(signature, "$acn", string(g.actionClassName)),
    switchVar
   );
  FOREACH_OBJLIST(Symbol, syms, symIter) {
    ...
    if ((whichFunc==0 && sym.dupCode) ||
      ...
      format_to(out,
        "    case {}:\n"
        "{}",
        sym.getTermOrNontermIndex(),
        replace(replace(templateCode,
  ...
  switch (whichFunc) {
    ...
    case 0:    // unspecified dup
      if (!g.useGCDefaults) {
        ...
        out.append("      return (SemanticValue)0;\n");
      }
      else {
        // using GC, sharing is fine
        out.append("      return sval;\n");
      }
      break;
    ...
    case 1:    // unspecified del
      if (!g.useGCDefaults) {
        ...
        if (syms.firstC()->isNonterminal()) {
          ...
          format_to(out,
            "      int arrayMin = 0;\n"
            "      int arrayMax = {syms_count};\n"
            "      xassert({switchVar} >= arrayMin && {switchVar} < arrayMax);\n"
            "      std::cout << \"WARNING: there is no action to deallocate terminal \"\n"
            "        << termNames[{switchVar}] << std::endl;\n",
            fmt::arg("syms_count", syms.count()),
            fmt::arg("switchVar", switchVar)
          );
        }
      }
      else {
    ...
    case 2: {  // unspecified merge: warn, but then use left (arbitrarily)
      format_to(out,
        ...
        "           << \": {}there is no action to merge nonterm \"\n"
        "           << nontermNames[{}] << std::endl;\n"
        "      {};\n",
        g.defaultMergeAborts ? "error: " : "WARNING: ",
        switchVar,
        g.defaultMergeAborts ? "abort()" : "return left"
      );
      break;
    }
    ...
    case 4:    // unspecified classifier: identity map
      out.append("      return oldTokenType;\n");
      break;
  }
  out.append(
    "  }\n"
